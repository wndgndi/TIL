...
class Solution {
    public int solution(int[] number) {
        ...
        for(int i=0; i<number.length; i++) {   // 0부터 number의 크기만큼 반복
           for(int j = i+1; j<number.length; j++) {   // i+1 부터 number의 크기만큼 반복
               for(int k = j+1; k<number.length; k++) {   // j+1 부터 number의 크기만큼 반복
                   if(number[i] + number[j] + number[k] == 0) {  // number의 i, j, k 번째 인덱스를 더했을 때 0이면,
