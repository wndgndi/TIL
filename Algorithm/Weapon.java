...
기사단원의 수를 나타내는 정수 number와 이웃나라와 협약으로 정해진 공격력의 제한수치를 나타내는 정수 limit와 제한수치를 초과한 기사가 사용할 무기의 공격력을 나타내는 
정수 power가 주어졌을 때, 무기점의 주인이 무기를 모두 만들기 위해 필요한 철의 무게를 return 하는 solution 함수를 완성하시오.
...
class Solution {
    public int solution(int number, int limit, int power) {
        ...
        for(int i=1; i<=number; i++) {   //  number만큼 반복
            int cnt = 0;   // cnt를 0으로 초기화
            for(int j=1; j*j<=i; j++) {   //  약수를 구하는데 일일히 다 돌 필요 없으므로 제곱근만큼 반복
                if(i % j == 0) {   // i로 j를 나눈 나머지가 0이면,
                    cnt++;   // cnt가 1 증가
                    if(i / j != j) {   // i로 j를 나누었을 때 몫이 j가 아니면,
                        cnt++;   // cnt가 1 증가
                    }
                }
            }
            answer += cnt > limit ? power:cnt;   // cnt가 limit보다 크면 power를, 작으면 cnt를 answer에 더해줌    
        } 
