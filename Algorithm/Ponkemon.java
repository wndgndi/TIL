...
class Solution {
    public int solution(int[] nums) {
        int answer = 0;
        int a = nums.length/2;    // 최대 폰켓몬 개수
        List<Integer> list = new ArrayList<>();    // list 생성 
       
        for(int i=1; i<nums.length; i++) {   // 배열 nums 크기만큼 반복
            if(answer != a){    // answer 값이 a와 다를 경우,
                if(!list.contains(nums[i])) {   // list가 nums의 i번째 인덱스를 포함하지 않는다면,
                    list.add(nums[i]);   // list에 nums의 i번째 인덱스를 넣어줌
                    answer++;  // answer를 1 증가시킴
                }  
            } else if(answer == a) {    // answer와 a 값이 같은 경우,
                break;   // 최대값이므로 반복문 탈출
            } 
        }
...
class Solution {
    public int solution(int[] nums) {
        int answer = 0;
        int a = nums.length/2;    // 최대 폰켓몬 개수
        HashSet<Integer> hs = new HashSet<>();   // HashSet 생성
        
        for(int i=0; i<nums.length; i++) {   // 배열 nums의 크기만큼 반복
            hs.add(nums[i]);   // hs에 nums의 인덱스들을 넣어줌
        }
        
        answer = hs.size();   // answer의 값에 hs의 크기를 넣어줌
        
        if(answer > a) {    // answer의 값이 a보다 크면,
            answer = a;    // 최대값이 a이므로 answer를 a로 만들어줌
        }
